...
void inserir_na_fila(Fila *F, char *nome_usuario, int *erro) {
    ...
    novo->ponteiro_usuario->nome = (char *)malloc((strlen(nome_usuario) + 1) * sizeof(char));
    if (novo->ponteiro_usuario->nome == NULL) {
        free(novo->ponteiro_usuario);
        free(novo);
        *erro = 1;
    ...
    strcpy(novo->ponteiro_usuario->nome, nome_usuario);
...
// Após a chamada dar free(nome_aux)
char *remover_da_fila(Fila *F, int *erro) {
    ...
    // Alocar dinamicamente para o nome a ser retornado
    char *nome_aux = (char *)malloc((strlen(F->inicio->ponteiro_usuario->nome) + 1) * sizeof(char));
    if (nome_aux == NULL) {
        *erro = 1;
        return NULL;
    }

    // Copia o nome do usuário
    strcpy(nome_aux, F->inicio->ponteiro_usuario->nome);
    ...
    free(aux->ponteiro_usuario->nome); // Libera o nome
    free(aux->ponteiro_usuario);       // Libera a struct usuario
...
void excluir_fila(Fila *F, int *erro) {
    ...
    while (aux != NULL) {
        ...
        free(temp->ponteiro_usuario->nome);  // Libera o nome
        free(temp->ponteiro_usuario);        // Libera a estrutura usuario
